...
def getroll(x1,y1,x1_prev, y1_prev, yaw, z):
    ...
    if math.fabs(cor_x > 0.1) or math.fabs(cor_y) > 0.1:
        ...
        r = 0
    return r
def collisionavoidance(x1,y1,x2,y2,x3,y3,yaw,r,p,y,v,dur,mambo):
    # global total_time
    # r = 0
    # if total_time > 2:
    #     inital_con = True
    ...
    # #if (x1> 3.75) or (x1<-3.25) or (y1> 1.7) or (y1<-2.7) or (z1> 2.1):
    #print(total_time)
    distance1 = math.sqrt(math.pow((x2-x1), 2)+math.sqrt(math.pow((y2-y1), 2)))
    distance2 = math.sqrt(math.pow((x3-x1), 2)+math.sqrt(math.pow((y3-y1), 2)))

    if distance1 < 0.5:
        # if inital_con == True:
        mambo.fly_direct(roll=(0), pitch=(0), yaw=(0), vertical_movement=(0), duration=0.02)
            # inital_con = False
        dur= 0.03
        #print('x1 = ', x1 ,'y1 = ' ,y1 ,'z1 = ', z1)
        cor_x = x1-x2
...
#MAIN LOOP
    while not rospy.is_shutdown():
        ...
        roll, pitch, yaw3 = quaterion2euler(rx3, ry3, rz3, rw3)
        v = dovertical(z_3)
        ...
        y,yaw_diff = doyawangle(yaw3, y_3, desiredy, x_3, desiredx)
        pgain_pitch = pitchcontroller(x_3,y_3,desiredx, desiredy,prev_dis)
        ...
        if math.fabs(yaw_diff) < 45 and (z_3 > 0.7):
            p, prev_dis = doforward(x_3, desiredx, y_3, desiredy, pgain_pitch)
        else:
            p =0
        r = getroll(x_3,y_3, x3_prev, y3_prev, yaw3,z_3) #Input z to not use roll unless high enough
        r,p,y,v, dur  = collisionavoidance(x_3,y_3,x_1,y_1,x_5,y_5,yaw3,r,p,y,v,dur,mambo)
